...
# 1. 读取CSV文件
df = pd.read_csv(r"Data\Ori_data_set\Ship_relationship_table2.csv")

# 2. 指定需要编码的分类特征列
categorical_columns = ["ship_vessel_sub_type"]
...
# 6. 重命名二进制列
df_encoded.rename(
    columns={col: f"typecode{i+1}" for i, col in enumerate(binary_cols)},
    inplace=True
)
...
# 8. 将编码列移动到数据末尾
typecode_cols = [col for col in df_encoded if col.startswith("typecode")]
...
# 10. 保存结果
df_final.to_csv(r"Data\pre_processed_data_set\preType_data.csv", index=False)