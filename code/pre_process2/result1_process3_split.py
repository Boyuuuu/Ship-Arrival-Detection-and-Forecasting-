...
if __name__ == "__main__":
    config = {
        ...
        "重命名映射": {
            'SP_timezone_offset':'IP_SPTO',
            'IP_OH_SPT_*':'IP_OH_SCT_{num}',
            'SP_ctry_*':'IP_OH_SPPT_{num}',
            'EP_ctry_*':'OP_OH_EPPT_{num}'
        },
   ...
   # 处理数据
    df = smart_column_processor(
        r'Data\split_datasets\One_Hot_dataset\pridict_port\all_with_predictions_updated.csv',
        config
    )
      # 创建保存路径
    save_dir = Path("Data\split_datasets\One_Hot_dataset\pridict_port\splitdataset")
