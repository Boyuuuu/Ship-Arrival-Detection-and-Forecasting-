...
CONFIG = {
    "data_folder": r"Data\split_datasets\One_Hot_dataset\pridict_port\splitdataset",
    "model_save_path": r"Result\Predict_port\best_model.pth",
    "preprocessor_path": r"Result\Predict_port\preprocessor.joblib",
    "batch_size": 2048,
    "max_epochs": 800,
    "patience": 20,
    "metrics_plot_path": r"Result\Predict_port\training_metrics.png",
    "confusion_matrix_path": r"Result\Predict_port\confusion_matrix.png",
    "prf_curve_path": r"Result\Predict_port\prf_curves.png" , # 新增PRF曲线路径
    "index_table_path": r"Result\Predict_port\index_mapping.csv"
}
...
class FeatureEngineer:
    ...
    def transform(self, df: pd.DataFrame) -> np.ndarray:
        ...
        cat_features = df.filter(regex='IP_OH_VST|IP_OH_VT|IP_OH_SCT|IP_SPTO|IP_OH_SPPT|IP_ECT').values
...
class PortDataset(Dataset):
    def __init__(self, data_path: str, engineer: FeatureEngineer = None):
        ...
        # 获取目标列
        target_cols = raw_df.filter(regex='OP_OH_EPPT').columns  # 提前定义 target_cols
