...
from cityP_pridict_port_model_train import PortPredictor, FeatureEngineer, CONFIG
...
class PortDataset(Dataset):
    def __init__(self, data_path: str, engineer: FeatureEngineer = None):
        ...
        # 处理目标列（如果存在）
        target_cols = raw_df.filter(regex='OP_OH_EPPT').columns
    ...
    def _process_labels(self) -> torch.Tensor:
        target_cols = self.df.filter(regex='OP_OH_EPPT').columns
...
def predict(config):
    ...
    # 1. 加载模型
    model = PortPredictor(input_dim=1372, output_dim=468)
    ...
    dataset = PortDataset(
        data_path=r"Data\split_datasets\One_Hot_dataset\pridict_city\splitdataset\pridict_port\all_withoutOP.csv",
        engineer=engineer
    )
    ...
    df_processed['IP_OH_EPPT'] = final_predictions
...
if __name__ == "__main__":
    # 配置参数（需要与训练时一致）
    CONFIG = {
        "model_save_path": r"Result\Predict_port\best_model.pth",
        "preprocessor_path": r"Result\Predict_port\preprocessor.joblib"
    }
    
    # 执行预测并获取处理后的数据和预测结果
    input_data_path = r"Data\split_datasets\One_Hot_dataset\pridict_city\splitdataset\pridict_port\all_withoutOP.csv"
    output_data_path = r"Data\split_datasets\One_Hot_dataset\pridict_city\splitdataset\pridict_port\all_withoutOP_pridicted.csv"
    ...
    print(f"预测结果样例：{df_processed['IP_OH_EPPT'].head(10).tolist()}")